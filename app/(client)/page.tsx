...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Hero — compact typography and ICar Gezina primary blue */}
      <section className="relative isolate w-full overflow-hidden bg-slate-900">
        <div className="relative min-h-[600px] sm:min-h-[640px] lg:min-h-[680px] flex items-center">
          ...
          <div className="relative z-10 w-full mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-26 lg:pt-28 pb-28 sm:pb-32 lg:pb-36">
            <div className="w-full max-w-5xl mx-auto text-center">
              <h1 className="mx-auto max-w-4xl text-[2rem] sm:text-[2.65rem] lg:text-[3.1rem] font-extrabold tracking-[-0.025em] leading-[1.05] text-white">
                Find the car of your dreams at
                <span className="block mt-0.5">ICar Gezina</span>
              </h1>
              <p className="mx-auto mt-2 sm:mt-3 w-full max-w-4xl text-[13px] sm:text-sm lg:text-[15px] leading-5 sm:leading-6 text-white/95">
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredCars.map(car => <Link href={`/cars/${car.id}`} key={car.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow group border border-slate-200 flex flex-col"><div className="relative h-56 w-full overflow-hidden bg-slate-200"><div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent z-10 pointer-events-none"></div><Image src={car.imageUrl} alt={`${car.make} ${car.model}`} fill className="object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" unoptimized /><div className="absolute bottom-4 left-4 z-20 flex flex-col text-white"><div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 bg-[#0072bc] text-[10px] font-bold rounded uppercase">Featured</span><span className="text-sm font-medium opacity-90">{car.year}</span></div></div></div><div className="p-6 flex flex-col flex-1"><div className="text-sm font-semibold text-[#0072bc] mb-1 uppercase tracking-wider">{car.make}</div><h3 className="font-bold text-2xl italic text-slate-900 mb-2">{car.model}</h3><div className="text-2xl font-black text-slate-900 mb-4">R {car.price.toLocaleString()}</div><div className="mt-auto grid grid-cols-2 gap-4 border-t border-slate-100 pt-4"><div className="border-r border-slate-100"><span className="text-[10px] font-bold text-slate-400 uppercase block">Mileage</span><span className="text-sm font-bold text-slate-700 italic">{car.mileage.toLocaleString()} KM</span></div><div><span className="text-[10px] font-bold text-slate-400 uppercase block">Transmission</span><span className="text-sm font-bold text-slate-700 italic">{car.transmission}</span></div></div></div></Link>)}
          </div>
          <div className="mt-8 flex justify-center md:hidden"><Link href="/cars" className="bg-[#0072bc] px-6 py-3 rounded-lg text-white font-bold text-sm">View All Inventory</Link></div>
        </div>
      </section>
