...
export default async function ResourceDetailPage({ params }: { params: Promise<{ resource:string; id:string }> }) {
  ...
  if (!record) notFound()
  const { data: vehicle } = resource === 'leads' && record.car_id
    ? await supabase.from('cars').select('id,make,model,year,price,mileage,fuel_type,transmission,body_type,color,image_url').eq('id', record.car_id).maybeSingle()
    : { data: null }
