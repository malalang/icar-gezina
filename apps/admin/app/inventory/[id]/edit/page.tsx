...
export default async function EditVehiclePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white text-[#262626]">
      ...
      <section className="border-b border-[#e6e6e6] bg-[#1a2129] text-white"><div className="mx-auto max-w-[1440px] px-5 py-10 sm:px-8 sm:py-12 lg:px-12"><p className="text-[13px] font-bold uppercase tracking-[1.5px] text-[#bbbbbb]">Inventory / Vehicle editor</p><h1 className="mt-3 text-4xl font-bold leading-[1.08] sm:text-5xl">Edit {title}</h1><p className="mt-4 max-w-2xl text-base font-light leading-[1.55] text-[#bbbbbb]">Maintain the customer-facing listing, photography and vehicle specification from one precise workspace.</p><div className="mt-8 grid grid-cols-2 border-l border-t border-white/15 sm:grid-cols-4"><Summary label="Year" value={car.year || '—'} icon={<CalendarDays size={16} />} /><Summary label="Mileage" value={`${Number(car.mileage || 0).toLocaleString('en-ZA')} km`} icon={<Gauge size={16} />} /><Summary label="Fuel" value={car.fuel_type || '—'} icon={<Fuel size={16} />} /><Summary label="Price" value={`R ${Number(car.price || 0).toLocaleString('en-ZA')}`} icon={<Tag size={16} />} /></div></div></section>
