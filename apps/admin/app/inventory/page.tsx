  ...
  return <>
    ...
    <div className="table-wrap"><table><thead><tr><th>Vehicle</th><th>Year</th><th>Price</th><th>Mileage</th><th>Actions</th></tr></thead><tbody>
      ...
      {(vehicles ?? []).map(vehicle => <tr key={vehicle.id}>
        <td><div className="vehicle-cell"><div className="vehicle-thumb">{vehicle.image_url ? <img src={vehicle.image_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <CarFront size={20}/>}</div><div><div className="vehicle-name">{vehicle.make} {vehicle.model}</div><div className="vehicle-meta">{vehicle.body_type || 'Body type not set'} · {vehicle.fuel_type || 'Fuel not set'} · {vehicle.transmission || 'Transmission not set'}</div></div></div></td>
        <td>{vehicle.year || '—'}</td><td>R {Number(vehicle.price || 0).toLocaleString('en-ZA')}</td><td>{Number(vehicle.mileage || 0).toLocaleString('en-ZA')} km</td>
        <td><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Link href={`/inventory/${vehicle.id}`} className="button secondary" style={{padding:'8px 10px'}}><Eye size={14}/> View</Link><Link href={`/inventory/${vehicle.id}/edit`} className="button secondary" style={{padding:'8px 10px'}}>Edit</Link><DeleteVehicleButton id={vehicle.id} name={`${vehicle.make} ${vehicle.model}`}/></div></td>
      </tr>)}
