...
import { ArrowLeft, Check, Image as ImageIcon, Save, Sparkles, Trash2, X } from 'lucide-react'
...
export function VehicleForm({ values, action, submitLabel }: VehicleFormProps) {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to save vehicle.'
      // Next.js redirects are implemented as a thrown control-flow value. Let those pass through.
      if (message !== 'NEXT_REDIRECT') setError(message)
    } finally {
      setSaving(false)
      setSaved(true)
    }
  ...
  return (
    <form action={submit} className="vehicle-editor">
      ...
      <div className="editor-toolbar">
        <div>
          <Link href={values.id ? `/inventory/${values.id}` : '/inventory'} className="back-link"><ArrowLeft size={15} /> {values.id ? 'Vehicle details' : 'Vehicles'}</Link>
          <h2>{values.id ? `${values.make} ${values.model}` : 'New vehicle'}</h2>
          <p>Maintain the customer-facing listing from one place.</p>
        </div>
        <div className="editor-actions">
          <Link href={values.id ? `/inventory/${values.id}` : '/inventory'} className="button secondary">Cancel</Link>
          <button className="button" type="submit" disabled={saving}>
            <Save size={15} /> {saving ? 'Saving…' : saved ? 'Saved' : submitLabel}
          </button>
        </div>
      </div>
      ...
      <div className="editor-layout">
        <div className="editor-main">
          <section className="editor-card">
            <div className="editor-card-header"><div><h3>Vehicle details</h3><p>Core information shown to customers.</p></div><Sparkles size={18} /></div>
          ...
          <section className="editor-card">
            <div className="editor-card-header"><div><h3>Description</h3><p>Give the sales team a polished vehicle summary.</p></div></div>
            <label className="field field-wide"><span className="field-label">Vehicle description</span><textarea className="input field-textarea" name="description" defaultValue={values.description} rows={9} placeholder="Describe condition, specification, service history and standout features…" /></label>
          </section>

          <section className="editor-card">
            <div className="editor-card-header"><div><h3>Features & extras</h3><p>One feature per line. These are displayed as vehicle highlights.</p></div></div>
            <label className="field field-wide"><span className="field-label">Features</span><textarea className="input field-textarea" name="features" value={featuresText} onChange={e => setFeaturesText(e.target.value)} rows={9} placeholder={'M Sport package\nPanoramic roof\nLeather interior'} /></label>
          </section>

          <section className="editor-card">
            <div className="editor-card-header"><div><h3>Gallery</h3><p>One image URL per line. The cover image is kept separately.</p></div></div>
            <label className="field field-wide"><span className="field-label">Gallery image URLs <span className="field-hint">{gallery.length} image{gallery.length === 1 ? '' : 's'}</span></span><textarea className="input field-textarea" name="galleryUrls" value={galleryText} onChange={e => setGalleryText(e.target.value)} rows={8} placeholder="https://example.com/vehicle-front.jpg\nhttps://example.com/vehicle-side.jpg" /></label>
            {gallery.length > 0 && <div className="gallery-mini-grid">{gallery.slice(0, 8).map((url, index) => <div className="gallery-mini" key={`${url}-${index}`}><img src={url} alt={`Gallery ${index + 1}`} /><span>{index + 1}</span></div>)}</div>}
          </section>
        </div>

        <aside className="editor-side">
          <section className="editor-card">
            <div className="editor-card-header"><div><h3>Cover image</h3><p>Primary image used across the showroom.</p></div></div>
            <div className="image-preview large">{imageUrl ? <img src={imageUrl} alt={`${values.make} ${values.model} cover preview`} onError={e => { e.currentTarget.style.display = 'none' }} /> : <div><ImageIcon size={30}/><span>No cover image</span></div>}</div>
            <label className="field"><span className="field-label">Image URL</span><input className="input" name="imageUrl" value={imageUrl} onChange={e => setImageUrl(e.target.value)} placeholder="https://…" /></label>
          ...
          <section className="editor-card">
            ...
            <div className="checklist">{checklist.map(([label, ok]) => <span className={ok ? 'check-ok' : 'check-missing'} key={label}>{ok ? <Check size={14}/> : <span className="check-dot"/>}{label}</span>)}</div>
          </section>

          <section className="editor-card help-card">
            <h3>Save safely</h3>
            <p>Changes are written directly to the dealership inventory. Use the vehicle detail page to verify the final customer-facing listing.</p>
