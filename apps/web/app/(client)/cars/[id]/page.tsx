...
import { ArrowLeft, ArrowRight, CheckCircle2, Gauge, Fuel, ShieldCheck, Star, Tag, Settings2, CarFront } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[#2b2b2b] pb-20 text-white">
      <section className="relative isolate min-h-[345px] overflow-hidden bg-[#202020] pt-[86px] sm:min-h-[380px]">
        <Image src={car.imageUrl} alt={`${car.make} ${car.model}`} fill priority unoptimized referrerPolicy="no-referrer" className="object-cover object-center" />
        ...
        <div className="relative mx-auto flex min-h-[259px] max-w-7xl items-end px-4 pb-10 sm:px-6 lg:px-8 lg:pb-12">
          <div className="grid w-full items-end gap-8 lg:grid-cols-[1fr_auto_auto] lg:gap-12">
            <div className="min-w-0">
              ...
              <h1 className="max-w-3xl text-3xl font-bold leading-tight tracking-[-0.025em] text-white sm:text-4xl lg:text-[42px]">{car.year} {car.make} {car.model}</h1>
            </div>
            ...
            <Link href="/cars" className="inline-flex h-12 items-center justify-center rounded-xl bg-[#12a8b8] px-5 text-sm font-medium text-white shadow-lg shadow-black/20 transition hover:bg-[#0f98a7] hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-[#12a8b8] focus:ring-offset-2 focus:ring-offset-black/20 lg:mb-0.5">Explore Our Showroom</Link>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1.65fr)_minmax(340px,0.8fr)]">
          <div className="space-y-6">
            <section className="overflow-hidden rounded-xl bg-[#242424] shadow-sm">
              <div className="relative aspect-[16/9] min-h-[300px] overflow-hidden bg-[#202020] sm:min-h-[430px]">
                <Image src={car.imageUrl} alt={`${car.make} ${car.model}`} fill className="object-cover" referrerPolicy="no-referrer" priority unoptimized />
              ...
              {car.galleryUrls.length > 0 && <div className="grid grid-cols-4 gap-2 p-2 sm:grid-cols-5 lg:grid-cols-6">{car.galleryUrls.map((url, idx) => <div key={idx} className="group relative aspect-[4/3] overflow-hidden rounded-lg bg-[#333]"><Image src={url} alt={`${car.make} ${car.model} view ${idx + 1}`} fill className="object-cover transition duration-500 group-hover:scale-105" referrerPolicy="no-referrer" unoptimized /></div>)}</div>}
            </section>
            ...
            <section className="rounded-xl border border-white/10 bg-[#333333] p-6 shadow-sm sm:p-7">
              ...
              <p className="mt-4 text-sm leading-7 text-white/65">{car.description || `This ${car.year} ${car.make} ${car.model} is part of the ICar Gezina showroom catalogue.`}</p>
            </section>

            <section className="rounded-xl border border-white/10 bg-[#333333] p-6 shadow-sm sm:p-7">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#F28A2E]">Specifications</p>
              <h2 className="mt-2 text-2xl font-bold tracking-tight">Vehicle specifications</h2>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {specifications.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 border-b border-white/10 py-3"><span className="text-xs font-medium text-white/40">{label}</span><span className="text-right text-sm font-semibold text-white">{value}</span></div>)}
              </div>
            </section>

            <section className="rounded-xl border border-white/10 bg-[#333333] p-6 shadow-sm sm:p-7">
              <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
                <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#F28A2E]">Features & equipment</p><h2 className="mt-2 text-2xl font-bold tracking-tight">Features & equipment</h2></div>
                <span className="text-xs text-white/40">{car.features.length} listed features</span>
              </div>
              {car.features.length > 0 ? <div className="mt-6 grid gap-x-8 gap-y-3 sm:grid-cols-2">{car.features.map((feature, idx) => <div key={idx} className="flex items-start gap-2.5 border-b border-white/10 pb-3 text-sm text-white/70"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#F28A2E]" /><span>{feature}</span></div>)}</div> : <p className="mt-6 rounded-xl bg-white/5 p-5 text-sm text-white/45">No additional equipment has been recorded for this vehicle.</p>}
            </section>
            ...
            <section className="rounded-xl border border-white/10 bg-[#242424] p-6 text-white shadow-sm sm:p-7">
              <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                {car.parts.length > 0 && <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-bold uppercase tracking-[0.14em] text-white/60"><ShieldCheck className="h-4 w-4 text-[#F28A2E]" /> Recorded vehicle condition</div>}
              </div>
              {car.parts.length > 0 ? <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{car.parts.map((part, idx) => <div key={idx} className="rounded-xl border border-white/10 bg-white/[0.04] p-4"><div className="flex items-center justify-between gap-3"><span className="text-[9px] font-bold uppercase tracking-[0.14em] text-white/40">{part.name}</span><span className={`text-[10px] font-bold ${part.condition === 'Excellent' ? 'text-emerald-400' : part.condition === 'Good' ? 'text-sky-400' : 'text-amber-400'}`}>{part.condition}</span></div><p className="mt-2 text-xs leading-5 text-white/55">{part.description}</p></div>)}</div> : <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.04] p-5"><p className="text-sm leading-6 text-white/50">No vehicle health-check details are currently recorded for this vehicle. We will not infer or invent inspection results.</p></div>}
            </section>
          ...
          <aside id="finance" className="lg:sticky lg:top-28 lg:self-start">
            <div className="overflow-hidden rounded-xl border border-black/5 bg-white text-[#292929] shadow-xl">
              ...
              <div className="p-5 sm:p-6"><div className="mb-5 rounded-xl bg-[#eeeeee] p-4"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-black/35">Vehicle</p><p className="mt-1 font-bold">{car.year} {car.make} {car.model}</p><p className="mt-1 text-xl font-black">R {car.price.toLocaleString()}</p></div><CarLeadForms carId={car.id} /></div>
