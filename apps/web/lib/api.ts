...
export async function getCars(): Promise<Car[]> {
  ...
  if (error) throw new Error(`Unable to load vehicles from Supabase: ${error.message}`);
...
export async function getCarById(id: string): Promise<Car | undefined> {
  ...
  if (error) throw new Error(`Unable to load vehicle from Supabase: ${error.message}`);
